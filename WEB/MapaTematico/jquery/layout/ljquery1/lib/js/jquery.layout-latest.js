...
,	_c = {
	...
	,	panes: { // defaults for ALL panes - overridden by 'per-pane settings' below
		...
		,	cssDemo: { // DEMO CSS - applied if: options.PANE.applyDemoStyles=true
				padding:	"2px" // aqui modificar para el espacio entre paneles
			,	background:	"#FFF"
			,	border:		"1px solid #1C70C4"
			...
			if ($T) {
				...
				if (dir == "horz") { // north/south
					...
					$T.children(".content").each(function(){
						$TC = $(this);
						$TC.css("marginLeft", Math.floor(((width-$TC.outerWidth())/2)-0)); // could be negative
					});
				}
				else { // east/west
					...
					$T.children(".content").each(function(){
						$TC = $(this);
						$TC.css("marginTop", Math.floor(((height-$TC.outerHeight())/2)-0)); // could be negative
					});
				}
